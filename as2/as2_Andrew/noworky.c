...
{
	int i; // bug was here: because the variable i was made into the type of unsigned int, when the value would decrement below zero, the value would become a large value instead,
	// this would cause the for loop to try and access an index in the arrays that does not exist, and cause a segmentation fault. The fix was to make the unsigned int, just be the 
	// type int, and this would allow the value to go negative when decrementing from 0.
	for (i = size-1; i >= 0; i--) { 
